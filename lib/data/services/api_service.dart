...
class ApiService {
  ...
  Future<List<MonumentModel>> getMonuments() async {
    ...
    print('DEBUG: API Request -> GET $url');
    
    try {
      final response = await http.get(url).timeout(const Duration(seconds: 5));
      print('DEBUG: API Response [${response.statusCode}] for /monuments');
      ...
      print('DEBUG: API Exception in getMonuments: $e');
      rethrow;
    }
  }
  ...
  Future<MonumentModel> getMonumentDetail(String id) async {
    ...
    try {
      final response = await http.get(url).timeout(const Duration(seconds: 5));
      print('DEBUG: API Response [${response.statusCode}] for /monuments/$id');
      ...
      print('DEBUG: API Exception in getMonumentDetail: $e');
      rethrow;
    }
  }
}
